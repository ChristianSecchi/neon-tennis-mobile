...
func _input(event):
	if event is InputEventScreenDrag:
		if event.position.x <= 315:
			paddleOrange.position.y = event.position.y
		
		if event.position.x >= 1605:
			paddleBlue.position.y = event.position.y

func _physics_process(delta):
	
	if paddleOrange.position.x != 228:
		paddleOrange.position.x = 288
	
	if paddleBlue.position.x != 1632:
		paddleBlue.position.x = 1632
	
	paddleBlue.move_and_slide(Vector2(0,0))
	paddleOrange.move_and_slide(Vector2(0,0))
