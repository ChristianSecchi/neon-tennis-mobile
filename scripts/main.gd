...
func _input(event):
	if event is InputEventScreenDrag:
		if event.position.x <= 315:
			paddle1.position.y = event.position.y
		
		if event.position.x >= 1605:
			paddle2.position.y = event.position.y

func _physics_process(_delta):
	
	if paddle1.position.x != 228:
		paddle1.position.x = 288
	
	if paddle2.position.x != 1632:
		paddle2.position.x = 1632
	
	paddle1.move_and_slide(Vector2(0,0))
	paddle2.move_and_slide(Vector2(0,0))
...
func loss_handling():
	...
	if lost:
		...
		if SinglePlayerLogic.score > Settings.topScore:
			Settings.topScore = SinglePlayerLogic.score
